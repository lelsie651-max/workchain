...
def _make_v9_db(path):
    ...
    try:
        ...
        db_module._create_v5_schema(conn)
        db_module._create_v6_schema(conn)
        db_module._create_v7_schema(conn)
        db_module._create_v8_schema(conn)
        db_module._create_v9_schema(conn)
        db_module._set_schema_version(conn, 9)
        conn.commit()
    finally:
        conn.close()
...
def test_init_db_creates_all_tables(db_file):
    ...
    rows = conn.execute(
        ...
        WHERE type = 'table' AND name IN (
            ...
            'event_match_runs', 'event_change_runs', 'event_changes', 'source_reviews'
        )
        ORDER BY name
        """
    ).fetchall()
...
def test_v9_database_migrates_to_v10_and_keeps_existing_rows(db_file):
    _make_v9_db(db_file)
    ...
    try:
        ...
        conn.execute(
            """
            INSERT INTO evidence_extractions (
                extraction_id, evidence_id, origin, provider, model, transcript,
                observations, warnings, created_at, supersedes_extraction_id
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                "ext-1",
    ...
    try:
        ...
        source_reviews_table = reopened.execute(
            "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'source_reviews'"
        ).fetchone()
        assert extraction["extraction_id"] == "ext-1"
        assert source_reviews_table["name"] == "source_reviews"
    finally:
        reopened.close()
